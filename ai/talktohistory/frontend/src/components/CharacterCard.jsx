...
export default function CharacterCard({ character }) {
  ...
  return (
    ...
    >
      <div className={`relative h-40 bg-gradient-to-br ${character.color} flex items-center justify-center overflow-hidden`}>
        ...
        <button
          type="button"
          onClick={onFav}
          className="absolute top-3 left-3 w-9 h-9 rounded-full bg-white/90 text-lg flex items-center justify-center shadow-sm hover:scale-110 transition-transform"
          title={fav ? "Unfavorite" : "Favorite"}
        >
          {fav ? "❤️" : "🤍"}
        </button>
        <div className="absolute top-3 right-3 bg-white/80 backdrop-blur-sm text-dark text-xs font-semibold px-2.5 py-1 rounded-full">
          {character.vibe}
        </div>
