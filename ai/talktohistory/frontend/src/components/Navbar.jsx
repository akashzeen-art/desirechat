      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
            ...
            {links.map((l) =>
              l.onClick ? (
                <button
                  key={l.label}
                  onClick={l.onClick}
                  className="px-4 py-2 rounded-xl text-sm font-medium text-muted hover:text-dark hover:bg-primary/5 transition-all duration-200"
                >
                  {l.label}
                </button>
              ) : (
                <Link
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                    isActive(l.to)
                      ...
                      : "text-muted hover:text-dark hover:bg-primary/5"
                  }`}
                >
                  {l.label}
                </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              className="md:hidden text-muted hover:text-dark p-2 rounded-lg hover:bg-primary/5 transition-colors"
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-primary/10 py-3 space-y-1 animate-fade-in">
            {links.map((l) =>
              l.onClick ? (
                <button
                  key={l.label}
                  onClick={l.onClick}
                  className="block w-full text-left px-4 py-2.5 text-sm rounded-xl text-muted hover:text-dark hover:bg-primary/5"
                >
                  {l.label}
                </button>
              ) : (
